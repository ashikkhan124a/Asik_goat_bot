...
module.exports = {
  config: {
    ...
    countDown: 20,
    role: 0,
    shortDescription: "Generate a custom meme image using mentioned user's profile picture",
    longDescription: "",
    category: "nsfw",
    guide: "{pn} @mention"
  },
  ...
  onStart: async function ({ message, event }) {
    ...
    if (mention.length == 0) {
      return message.reply("দয়া করে কাউকে মেনশন করুন! যেমন: /fak @User");
      ...
      bal(one, two).then(ptth => {
        ...
        console.error(error);
        message.reply("ছবি তৈরি করতে ব্যর্থ হয়েছে!");
      });
    } else {
      const one = mention[1];
      const two = mention[0];
